...
__global__ void GPUfnv2(char ** query, char ** reference, int *** sc_mat_list, char *** dir_mat_list, int * res, char ** simple_rev_cigar) {    
	int n = blockIdx.x;
	...
	for (int i = 1; i < S_LEN; i++) {
		...
		currScore = setDirScore(query[n], reference[n], sc_mat, dir_mat, i, i);
		// Handle vertical/horizontal cells 
		for(int j = i+1; j < S_LEN; j++) {
			if(threadIdx.x == 0) {
				currScore = setDirScore(query[n], reference[n], sc_mat, dir_mat, i, j);
				counterx++;
			} else {
				currScore = setDirScore(query[n], reference[n], sc_mat, dir_mat, j, i);
				countery++;
			}
		}
		__syncthreads();
	}
	printf("%d %d %d\n",threadIdx.x, counterx, countery);
}

__global__ void GPUfn(char ** query, char ** reference, int *** sc_mat_list, char *** dir_mat_list, int * res, char ** simple_rev_cigar) {    
...
int main(int argc, char * argv[]) {
    ...
    GPUfnv2<<<blocksPerGrid, threadsPerBlock>>>(d_query, d_reference, d_sc_mat_list, d_dir_mat_list, d_res, d_simple_rev_cigar);
    CHECK_KERNELCALL();
